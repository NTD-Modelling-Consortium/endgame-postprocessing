...
if __name__ == "__main__":
    ...
    # all_files = sorted(list(file_util.get_flat_regex(
    #     r"ntdmc-(?P<iu_id>(?P<country>[A-Z]{3})\d{5})-(?P<scenario>scenario_\w+)-200_sims.csv",
    #     input_dir,
    # )))
    all_files = sorted(
        list(
            file_util.get_flat_regex(
                r"ntdmc-(?P<iu_id>(?P<country>TGO)48688)-(?P<scenario>old_scenario_-1)-200_sims.csv",
                input_dir,
            )
        )
    )
    ...
    all_warnings = [[w for w in warning_list] for warning_list in warnings_from_each_iu]
    output_dir = "local_data/epioncho-2025-03-04"
    with CollectAndPrintWarnings() as collected_warnings:
