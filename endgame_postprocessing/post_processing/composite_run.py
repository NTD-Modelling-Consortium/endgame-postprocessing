...
def _compute_year_range_intersection(
    canonical_iu_runs: List[pd.DataFrame], iu_data: IUData
...
def _trim_canonical_ius_to_year_range(
    canonical_iu_runs: List[pd.DataFrame], year_range: Tuple[int, int]
) -> List[pd.DataFrame]:
    """
...
def _get_priority_populations(ius: List[pd.DataFrame], iu_metadata: IUData):
    ...
    Args:
        ius (list[pd.DataFrame]): A list of DataFrames, each representing an IU,
            containing IU-specific data.
        iu_metadata (IUData): An IUData object that provides metadata and helper methods for IUs.

    Returns:
        np.ndarray: A 3D numpy array where each element represents the yearly priority
                    population for an IU.
...
def build_composite_run(
    ...
    """
    Build a composite run by aggregating disease prevalence data across multiple
    Implementation Units (IUs).
    ...
    year_intersection = _compute_year_range_intersection(
        canonical_iu_runs, iu_data
    )
    if year_intersection is None:
        raise ValueError(
            "No overlap between simulation years and population metadata years"
        )

    canonical_iu_runs = _trim_canonical_ius_to_year_range(
        canonical_iu_runs, year_intersection
    )

    if not canonical_iu_runs:
        raise ValueError(
            "No IU data remaining after trimming to metadata year range"
        )

    # Assumptions: same number of draws in each IU run
    # Same year IDs in each one
    draw_column_names, all_ius_draws = canonical_columns.extract_draws(
        canonical_iu_runs
    )
    ...
    priority_populations = _get_priority_populations(
        canonical_iu_runs, iu_data
    )
    ...
    total_population = np.array(
        list(itertools.islice(total_population_iter, num_years))
    ).reshape((-1, 1))
