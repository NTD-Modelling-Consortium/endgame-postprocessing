...
def test_trim_canonical_ius_to_year_range_perfect_overlap():
    ...
    from endgame_postprocessing.post_processing.composite_run import (
        _trim_canonical_ius_to_year_range
    )
...
def test_build_composite_run_with_year_mismatch_no_overlap_raises_error():
    ...
    with pytest.raises(
        ValueError,
        match="No overlap between simulation years and population metadata years"
    ):
...
def test_build_composite_run_with_metadata_extends_beyond_simulation():
    ...
    population_data = IUData(
        create_dummy_population_file_for_disease_with_years(Disease.LF, {
            "AAA00001": {
                2008: 80, 2009: 90, 2010: 100, 2011: 110,
                2012: 120, 2013: 130, 2014: 140, 2015: 150
            }
        }),
        Disease.LF,
        IUSelectionCriteria.ALL_IUS
...
def _compute_prevalences_in_country(canonical_ius: List[pd.DataFrame],
                                    population_data: IUData):
    """
    Compute prevalence in a country by aggregating disease data across multiple
    ...
    ius_org_by_scenario = itertools.groupby(
        canonical_ius, key=lambda iu: iu[canonical_columns.SCENARIO].iloc[0]
    )
    ...
    for _, ius in ius_org_by_scenario:
        ...
        # Use islice to get the right number of years for both longitudinal
        # and non-longitudinal data
...
def test_compute_year_range_intersection_longitudinal_data_perfect_overlap():
    ...
    from endgame_postprocessing.post_processing.composite_run import (
        _compute_year_range_intersection
    )
