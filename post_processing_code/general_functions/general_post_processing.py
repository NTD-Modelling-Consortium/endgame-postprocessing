...
def custom_progress_bar_update(
    progress_bar, 
    curr_index: int, 
    total: int
):
...
def post_process_file_generator(
    ...
    """
    Returns a generator for files in a given directory, only returning files that end with a certain string.

    Args:
        file_directory (str): The name of the file directory all the files are in. Should be in the format file_directory/scenario/country/iu/output_file.csv.
        end_of_file (str): A substring that defines the files to be processed. Default is ".csv".

    Returns:
        Yields a generator, which is a tuple, of form (scenario_index, total_scenarios, scenario, country, iu, full_file_path)
    """
            ...
            for iu in os.listdir(country_dir_path):
                ...
                for output_file in os.listdir(iu_dir_path):
                    if (output_file.endswith(end_of_file)): 
                        yield (scenario_index, total_scenarios, scenario, country, iu, os.path.join(file_directory, scenario, country, iu, output_file))
...
def measure_summary_float(
    ...
    """
    The default summary calculation for a float measure, including calculation of mean, median, std, and percentiles.
    ...
    Returns:
        Returns a summarized (using build_summary) 2D matrix where each column represents a unique summarized value.
    """
    ...
    return build_summary(
        ...
        percentiles_dict={k: np.percentile(values, k, axis=1) for k in PERCENTILES_TO_CALC},
...
def extract_percentiles(
    ...
    """
    Helper function to extract percentiles from a dictionary of percentile names to a list of values into a 2d matrix
    The columns denote each percentile (i.e col 0 = 2.5 percentile, col 1 = 5th percentile, etc.)
    
    Args:
        percentiles_dict (dict[str: list[int]]): A dictionary of percentile_name: a list of percentile calculations with that name.
        percentile_names (list[str]): A list of percentile names in the order in which the columns should appear.
    
    Returns:
        Returns a 2D matrix where columns denote each percentile (i.e col 0 = 2.5 percentile, col 1 = 5th percentile, etc.)
    """
    assert len(percentile_names) > 0 and len(percentile_names) <= len(percentiles_dict.keys())
    output_array = np.empty([len(percentiles_dict[percentile_names[0]]), len(percentile_names)])
    for index, percentile_name in enumerate(percentile_names):
        ...
        if (len(percentile_list) != output_array.shape[0]):
            raise ValueError(f'Percentile size is not consistant, expected size {output_array.shape[0]}, but got size {len(percentile_list)} for key {percentile_name}')
        output_array[:, index] = percentiles_dict[percentile_name]
    return output_array


def build_summary(year_id: list[int],
                  age_start: list[int], 
                  age_end: list[int],
    ...
    """
    Helper function to build a standardized summary "block" based on the input. All inputs should be of the same length.
    ...
    Args:
        ...
        percentiles_dict (list[int]): A dictionary of percentile_name: a list of percentile calculations with that name.
        percentile_name_order (list[int]): A list of percentile names in the order in which the columns should appear.
    ...
    Returns:
        Returns a 2D matrix where columns denote each value passed in. (i.e col 0 = year_id, col 1 = age_start, etc.)
    """
    if not(len(year_id) == len(age_start) == len(age_end) == len(measure_name) == len(mean) == len(standard_deviation) == len(median)):
...
def calc_prob_under_threshold(
    ...
    Args:
        prevalence_vals (np.ndarray): A 2D matrix where columns are different runs, and rows are different years
        threshold (float): The number at which a threshold is considered to be passed 
    
    Returns:
        A matrix of shape [length of the number of years in the input, 1] where the values are the probability of that year reaching the threshold.
    """
...
def find_year_reaching_threshold(
    ...
    Args:
        ...
        comparitor_function (function): The numpy comparator function to be used. By default this is np.less.
    
    Returns:
        The year in which the threshold was reached, or NaN if it wasn't.
    """
    indeces_of_meeting_threshold = np.where(comparitor_function(comparison_prevalence_values, threshold))[0]
...
def filter_out_old_data(
    ...
    post_processing_start_mask = raw_model_outputs[:, year_column_loc].astype(float) >= post_processing_start_time
    return raw_model_outputs[post_processing_start_mask, :]
...
def calculate_probabilities_and_thresholds(
    ...
    Args:
        filtered_model_outputs (pd.DataFrame): The filtered data output in the form of a dataframe from multiple runs of the model.
    ...
    prevalence_mask = filtered_model_outputs[:, measure_column_loc] == prevalence_marker_name
    ...
    prevalence_vals = filtered_model_outputs[prevalence_mask, :][:, draws_loc].astype(float)

    prob_prevalence_under_threshold = calc_prob_under_threshold(prevalence_vals, threshold)
    ...
    year_of_90_under_threshold = find_year_reaching_threshold(prob_prevalence_under_threshold, pct_runs_under_threshold, filtered_model_outputs[prevalence_mask, year_column_loc], np.greater_equal)
    ...
    year_of_threshold_prevalence_avg = find_year_reaching_threshold(np.mean(prevalence_vals, axis=1), threshold,  filtered_model_outputs[prevalence_mask, year_column_loc], np.less)
...
def summarize_measures(
    ...
    Args:
        raw_output_data (pd.DataFrame): The raw data output in the form of a dataframe from multiple runs of the model.
        year_column_loc (int): Location of the year column.
        measure_column_loc (int): Location of the measure column.
        age_start_column_loc (int): Location of the age_start column.
        age_end_column_loc (int): Location of the age_end column.
        draws_loc list(int): Location of the draw columns.
        prevalence_marker_name (str): The name of the prevalence measure that is used to compare with the threhsold.
        measure_summary_map (dict[str: function]): a map of measure names to a summary calculation functions. This will be used to calculate the mean, median, std, and percentiles.
                                                                By default it is `None`, which will summarize the measure passed in `prevalence_marker_name` using the `measure_summary_float` function.
    ...
    if (measure_summary_map == None) | (prevalence_marker_name not in measure_summary_map.keys()):
        measure_summary_map[prevalence_marker_name] = measure_summary_float
    # replacing all `None` with the default function
    measure_summary_map = {key: (func if func is not None else measure_summary_float) for key, func in measure_summary_map.items()}

    # loop through each measure key and calculate the summary for it independently, using either the provided function or the default one
    for key, func_to_summarize in measure_summary_map.items():
        ...
        if not(callable(func_to_summarize)):
            raise ValueError(f'Value for key {key} in measure_summary_map is not a callable function. Please provide a function, or set the value to None to use the default function')
        
        if measure_summaries_output == None:
            measure_summaries_output = func_to_summarize(raw_output_data[tmp_measure_mask, :], year_column_loc, age_start_column_loc, age_end_column_loc, measure_column_loc, draws_loc)
        else:
            measure_summaries_output = np.row_stack((
                measure_summaries_output, 
                func_to_summarize(raw_output_data[tmp_measure_mask, :], [year_column_loc, age_start_column_loc, age_end_column_loc, measure_column_loc], draws_loc)
                ))
...
def process_single_file(
    ...
    Args:
        raw_model_outputs (pd.DataFrame): The raw data output in the form of a dataframe from multiple runs of the model.
    ...
    # Making sure we start the calculations from where we want
    filtered_model_outputs = filter_out_old_data(raw_model_outputs.to_numpy(), year_column_loc, post_processing_start_time)
    probabilities_and_threshold_outputs = calculate_probabilities_and_thresholds(filtered_model_outputs, year_column_loc, measure_column_loc, age_start_column_loc, age_end_column_loc, draws_loc, prevalence_marker_name, threshold, pct_runs_under_threshold)
    
    summarized_measure_outputs = summarize_measures(raw_model_outputs.to_numpy(), year_column_loc, measure_column_loc, age_start_column_loc, age_end_column_loc, draws_loc, prevalence_marker_name, measure_summary_map)
    
    # combine all the outputs together
    output = np.row_stack(
        (
            summarized_measure_outputs,
            probabilities_and_threshold_outputs
        )
    )
