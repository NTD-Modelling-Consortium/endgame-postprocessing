...
def aggregate_post_processed_files(
    ...
    """
    Combines all data outputs in a given folder and filters as necessary. The format of the csv's in the folder should be the same output format of a call to `process_single_file`
...
def iu_lvl_aggregate(
    ...
    """
    A wrapper function for aggregate_post_processed_files that takes in a path to where the data is located, and returns
    aggregated iu-lvl data, with columns modified to the correct types, and filtered if requested.
    ...
    if len(filter_measures) > 0:
        aggregated_data.loc[aggregated_data[measure_column_name].isin(filter_measures), :]

    # replace empty strings with nan/none where required, and properly type the columns
    aggregated_data.loc[:, columns_to_replace_with_nan] = aggregated_data.loc[:, columns_to_replace_with_nan].replace("", np.nan)
    aggregated_data.loc[:, ~aggregated_data.columns.isin(columns_to_replace_with_nan)] = aggregated_data.loc[:, ~aggregated_data.columns.isin(columns_to_replace_with_nan)].replace("", None)
...
def country_lvl_aggregate(
    ...
    Args:
        ...
        measure_column_name (str): the column where the measure names are located. 
                                                Examle: 'measure'.
        general_summary_cols (list[str]): a list of measures that we want to generally summarize with mean, percentiles, std, and median. 
                                                    Examle: ["prevalence", "year_of_1_mfp_avg", "year_of_90_under_1_mfp"].
        general_groupby_cols (list[str]): a list of columns that we want to use to group the general summaries by.
                                                    Examle: ["scenario", "country_code", "year_id", "age_start", "age_end", "measure"].
        threshold_summary_cols (list[str]): a list of measures that we want to calculate the threshold statistics for (number of IU's that reach a threshold, calculated by the pct of non-null values).
                                                    Examle: ["year_of_1_mfp_avg", "year_of_90_under_1_mfp"].
        threshold_groupby_cols (list[str]): a list of measures that we want to use to group the threshold statistics by.
                                                    Examle: ["scenario", "country_code", "measure"].
        threshold_cols_rename (dict): a dictionary of {"measure_name": "new_measure_name"} that will rename measures used to calculate the threshold statistics.
                                                   Examle: {"year_of_90_under_1_mfp":"pct_of_ius_passing_90pct_threshold", "year_of_1_mfp_avg":"pct_of_ius_passing_avg_threshold"}
    
    Returns:
        A dataframe with country-level metrics
    """
    general_summary_df = iu_lvl_data.loc[iu_lvl_data[measure_column_name].isin(general_summary_cols), :].copy()
    ...
    general_summary = general_summary_df.groupby(general_groupby_cols).agg({
        "mean": ["mean"] +
                [percentile(p) for p in PERCENTILES_TO_CALC] +
                 #percentile(2.5), percentile(5), percentile(10), percentile(25), percentile(50), percentile(75), percentile(90), percentile(95), percentile(97.5),
                [np.std, "median"]
    }).reset_index()
    general_summary.columns = general_groupby_cols + ["mean"] + [str(p) + "_percentile" for p in PERCENTILES_TO_CALC] + ["standard_deviation", "median"]
    general_summary[general_groupby_cols] = general_summary[general_groupby_cols].replace(-1, np.nan)

    summarize_threshold = iu_lvl_data[iu_lvl_data["measure"].isin(threshold_summary_cols)].groupby(threshold_groupby_cols).agg({
...
def africa_lvl_aggregate(
    ...
    Args:
        ...
        measure_column_name (str): the column where the measure names are located. 
                                                Examle: 'measure'.
        measures_to_summarize (list[str]): a list of measures that we want to generally summarize with mean, percentiles, std, and median. 
                                                    Examle: ["prevalence"].
        columns_to_group_by (list[str]): a list of columns that we want to use to group the summaries by.
                                                    Examle: ["scenario", "year_id", "age_start", "age_end", "measure"].
    
    Returns:
        A dataframe with africa-level metrics
    """
    africa_summary = country_lvl_data[country_lvl_data[measure_column_name].isin(measures_to_summarize)].groupby(columns_to_group_by).agg({
        "mean": ["mean"] +
                [percentile(p) for p in PERCENTILES_TO_CALC] +
                [np.std, "median"]
    }).reset_index()

    africa_summary.columns = columns_to_group_by + ["mean"] + [str(p) + "_percentile" for p in PERCENTILES_TO_CALC] + ["standard_deviation", "median"]
